...
void reset_fields(grackle_field_data  *my_fields, code_units *my_units){
  /*********************************************************************
  ...
  *********************************************************************/
  ...
  for (i = 0;i < field_size;i++) {
    ...
    my_fields->HI_density[i] = my_fields->density[i]*my_units->density_units/mh;
    ...
    // initilize internal energy (here 100 K for no reason)
...
int main(int argc, char *argv[])
{
  ...
  //SETTING UP MAIN DATA STRUCTURES 
  code_units my_units;
  ...
  int N_iters = 1.e3;  //WE'LL GIVE THE NETWORK 1000 STEPS TO GET TO EQUILIBRIUM (THE NETWORK MAY SUBCYCLE THESE UP TO 5000 STEPS EACH)


   //initialize fields
   reset_fields(&my_fields, &my_units);
