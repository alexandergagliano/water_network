...
if __name__=="__main__":
    ...
    for i in range(4):
        ...
        pyplot.semilogy(np.log10( (data["HI"]+data["HII"]+data["H2I"]+data["H2II"]) / mass_hydrogen_cgs), data["temperature"], label ='log(Z/Z$_{\odot}$) = %d, Sim' % logZ[i], color=colors[i])
        if i == 0:
    ...
    pyplot.tight_layout()
    pyplot.savefig("%s_Water.png" % (output))
