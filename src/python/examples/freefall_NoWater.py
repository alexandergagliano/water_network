...
if __name__=="__main__":
    ...
    for j in range(5):
        ...
        initial_temperature = 5000. # start the gas at this temperature
        # then begin collapse
        initial_density     = 1.0e-1 * mass_hydrogen_cgs # g / cm^3
        # stopping condition
        final_density       = 1.e6 * mass_hydrogen_cgs
        ...
        fc["de"][:] = 2.e-4 * mass_electron_cgs / mass_hydrogen_cgs * fc["density"]
        ...
        data0 = evolve_constant_density(
            ...
            safety_factor=safety_factor/10.) 
       
        fc["energy"][:] = cooling_temperature / \
            fc.chemistry_data.temperature_units
        ...
        pyplot.semilogy(np.log10(data["density"]/mass_hydrogen_cgs), data["temperature"], label ='log(Z/Z$_{\odot}$) = -inf, 3body = %d' % j, color=colors[j])
        if i == 0:
        ...
        with open('T_vs_logn__Z=%d_WaterOmukai2005.csv' % logZ[i], 'r') as omukaiData:
                ...
                pyplot.semilogy(Omukai_n, Omukai_T, label='log (Z/Z$_{\odot}$) = %d, Omukai' % logZ[i], color=colors[i], linestyle=':')

                Omukai_n = []
                Omukai_T = []

    pyplot.ylim(ymax = 1.e4)
    pyplot.xlim(xmin = -1.5, xmax=6)
    leg = pyplot.legend(fancybox = True, labelspacing=0.0, loc='lower right')
    leg.get_frame().set_alpha(0.5)
 
    #p2, = pyplot.plot(np.log10( data["density"] / mass_hydrogen_cgs ), np.log10( data["H2I"] / data["density"] ),
    #                    color="red")
    ...
    pyplot.savefig("%s_NoWater.png" % (output))
